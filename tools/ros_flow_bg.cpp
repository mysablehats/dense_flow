...
image_transport::Publisher pubc;

Mat flow_img_x;
Mat flow_img_y;
Mat flow_img_z;
Mat flow_img_c, combflow;
vector<Mat> channels;
...
int main(int argc, char** argv){
  ...
  pubc = local_it.advertise("flow_xy", 1);
          ...
          try
            {
              ...
              if (!do_resize){
                  ...
                  flow_img_c.create(new_size, CV_8UC3);
              ...
              flow_img_z = Mat::zeros(new_size, CV_8UC1);
          ...
          try
            {
              ...
              channels.push_back(flow_img_x);
              channels.push_back(flow_img_y);
              channels.push_back(flow_img_z);
              ROS_INFO("size of channels x %d, %d",flow_img_x.cols ,flow_img_x.rows);
              ROS_INFO("size of channels y %d, %d",flow_img_y.cols ,flow_img_y.rows);
              ROS_INFO("size of channels z %d, %d",flow_img_z.cols ,flow_img_z.rows);
              ROS_INFO_STREAM("size of channels " << channels.size());
              cv::merge(&channels[0], 3,flow_img_c);
              //flow_img_c.convertTo();
              ROS_INFO("size of channels c %d, %d",flow_img_c.cols ,flow_img_c.rows);
              ...
              //there is something wrong with the cast.
  						...
  						puby.publish(msgy);
              sensor_msgs::ImagePtr msgc = cv_bridge::CvImage(std_msgs::Header(), "bgr8",flow_img_c).toImageMsg();
  						pubc.publish(msgc);
